package tw.org.by37.config;

public class RequestCode {
        public final static int FBLOGIN_REQUEST_CODE = 64206;

        public final static int MEMBER_ACTIVITY_CODE = 1001;
        public final static int SIGNUP_ACTIVITY_CODE = 2001;
        public final static int ORGANIZATION_ACTIVITY_CODE = 3001;
        public final static int SUPPLIESHELP_ACTIVITY_CODE = 4001;
        public final static int NEAR_ORGANIZATION_ACTIVITY_CODE = 5001;
        public final static int POSITION_ACTIVITY_CODE = 6001;
        public final static int SEARCH_ACTIVITY_CODE = 7001;

        public final static int REGISTER_SUCCESS_CODE = 1002;
        public final static int LOGIN_SUCCESS_CODE = 1003;
        public final static int LOGOUT_SUCCESS_CODE = 1004;

        public final static int SOURCE_CAMERA = 9501;
        public final static int SOURCE_PHOTO = 9502;
        public final static int CROP_PHOTO = 9503;

}
